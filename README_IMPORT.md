...
如果任务范围是全产品、多个一级模块或某个大模块，请先遍历所有可见一级菜单和二级菜单，必要时继续展开三级菜单，拿到菜单轮廓、页面清单和功能地图后，再输出分批设计计划，不得一次性生成完整测试用例；分批默认按一级模块下的三级菜单/页面域执行，三级菜单仍然过大时按小功能块继续拆分，三级菜单过小时才可与同一二级菜单下相邻三级菜单合并，跨三级菜单强依赖时按业务对象或业务链路合并成批，并写明拆分/合并原因；每批生成后必须分批回存，再做跨模块汇总。
