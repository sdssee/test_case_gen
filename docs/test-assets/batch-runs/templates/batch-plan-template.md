...
- 本次创建对象的后续生命周期复用同一测试数据 ID 和创建 owner 用例；每行使用对应 mutation plan 的 `交互实例ID`。
...
- 状态数量必须派生：页面/元素/覆盖/待确认数来自 discovery，功能用例数来自 manifest，异常、边界、权限/状态、数据一致性数按明确 DFX taxonomy 派生且允许重叠，禁止人工估算。
