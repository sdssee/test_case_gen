...
Agent 只能读取冻结输入并写自己的隔离 workspace。每个任务都通过 `agent-claim` 绑定 `task_id`、`execution_id` 和 source fingerprint；原生 sub-agent 还绑定物理 transcript。原生 Agent 派发连续失败时，supervisor 会把同一 claim 原子转换为哈希授权的 `codebuddy-isolated-fallback`，只开放该任务 output 白名单；`agent-submit`、Review 与 Delivery 仍重复验证同一批质量门禁。Review 未通过不得交付。

### 串行与并行

Discovery、Plan/DFX、Risk 和 Reviewer 始终串行。只有同一轮返回的 Case Worker 可以组成冻结 wave 并行执行；整波收齐后，成功结果按冻结顺序提交，失败或返工时先安全释放其余任务再提交控制结果。

如果 CodeBuddy 支持 sub-agent 但不支持后台并行，Case wave 仍先完整 claim，再按固定顺序逐个前台执行并暂存结果，收齐后统一决策。完全不支持 sub-agent 时也不询问用户、不停止：每个任务先记录一次原生重试，第二次失败后自动进入隔离 fallback，并保持原角色顺序、冻结 wave、逐元素 DFX、用例预算、独立 Reviewer 和单写者交付不变。降级只降低吞吐，不降低质量。
