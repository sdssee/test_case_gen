...
3. **页面深探**：只维护一份临时 `discovery-state.json`。按页面区域建立语义目标，执行基线盘点和业务流程；新状态用 `parent_id` 追加目标，相同目标去重。真实状态变化执行 `触发 → 状态出现 → 状态内操作 → 终态动作 → 页面/数据结果`。分页按专项规则拆分实际能力。
...
6. **Story 与原子场景**：重新读取 `case-design.md`，先按来源或独立业务价值与验收结果拆解并冻结 Story；再把事实抽象为角色/状态、动作/输入、数据、观察点和恢复路径并逐项核账。不得用页面元素、测试点或 DFX 反向增加 Story，不直接复制环境快照。
7. **DFX 评估**：事实核账后读取 `dfx-test-strategy.md`，为每个原子场景标记一个主 DFX，并补充适用质量场景。DFX 不得抽样、删除或合并不同事实；新产生理解问题时停止等待用户。
8. **用例设计**：以功能点为父场景连续编排。数据变更先形成一条完整成功主流程，再按有效值、边界、异常、只读和取消分支展开，不做字段笛卡尔积。用例从同一原子场景一次生成 `功能点-当前用例标题`、角色状态、入口导航、操作、终态、结果和恢复，完成后统一编号；每条用例独立造数和清理。
9. **写入前集中自查**：一次检查标题、编号、入口、特殊角色/状态、环境抽象、数据变更主流程、逐项值、临时交互闭环、步骤预期、场景/用例映射和枚举，只修正唯一结构化数据源。页面任务补全同一状态文件的 `scenario_case_mapping`。
10. **一次交付**：复制正式模板和导入模板，调用 `complete-deliverables`；页面任务传入 `--discovery-state`。正式测试设计只含 7 个标准 Sheet，输出到根目录 `deliverables/`。联合质检一次返回正式工作簿、深探映射和导入文件问题；失败停止，不自动修改、重生成或叠加 `fix_*`。
...
- 步骤从系统入口开始，导航使用 `一级菜单-二级菜单-目标页面`；UI 名称不加括号或引号。弹窗、抽屉、动态行、下拉和删除确认必须形成真实闭环。
