...
若原生 Agent 在开始执行前连续派发失败，使用同一 claim 和 execution 记录失败；第一次返回原生重试，第二次由 supervisor 原子授权隔离 fallback：

```powershell
scripts/run-test-design.ps1 agent-dispatch-failed `
  --run-dir <run-dir> `
  --task-id <task-id> `
  --execution-id <原claim执行ID> `
  --coordinator-id <coordinator-id> `
  --reason "<实际派发失败原因>" `
  --fallback-executor-id <角色唯一fallback执行ID> `
  --json
```
...
核心契约声明了 `codebuddy-subagent`、`codebuddy-isolated-fallback`、`codebuddy-main-session`、`codebuddy-agent-team` 和 `external-session`。正式执行只认证由项目 hook 约束的 sub-agent，以及 supervisor 在连续原生派发失败后从同一 claim 原子转换的 isolated fallback；fallback 不能直接 claim，其授权绑定原 execution、冻结输入、source fingerprint 和唯一 output 白名单。后三种身份仍不得用于正式生成、Review 或交付。调用方必须在首次领取前生成稳定唯一 `execution_id`；领取响应丢失时，用相同参数重试会返回同一 claim。
...
| 不支持 sub-agent，但页面探针和任务所需工具可用 | 原生派发重试一次后，supervisor 将同一 claim 转为隔离 fallback；按原角色和 wave 串行推进 | 自动继续；质量门禁不降级，吞吐下降 |
| 页面探针或任务必需工具不可用 | 不 claim Discovery 或返回外部阻塞 | 不能伪造实探事实 |

完全不支持 sub-agent 时，不把普通 `codebuddy-main-session` 伪装成正式执行身份。supervisor 只在两次派发失败均发生于执行开始前、且没有物理绑定、output、result 或 promotion 时，将原 claim 转成 `codebuddy-isolated-fallback`；项目 hook 随后只允许写该 task 的 allowlist output，禁止直接写正式账本、`artifacts/data`、编排状态或交付目录。fallback 产物经过相同 schema、晋升、累计阶段校验和独立逻辑 Review。`external-session`、Agent Team 与普通 main-session 仍未认证。
...
- 不支持 sub-agent 时必须走 supervisor 自动降级；不得直接用 `codebuddy-main-session`、`codebuddy-agent-team` 或 `external-session` 模拟正式成功。
