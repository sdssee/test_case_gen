...
21. 深探事实必须先按测试对象、角色/状态、动作/输入、数据、观察点和恢复路径形成并核账原子场景，再标记一个主 DFX；DFX 只能标记或补充，不得抽样、删除或合并不同事实。每项事实必须形成独立场景、说明合理合并依据，或明确落入性能设计、风险、自动化建议、不适用/不测范围；每个标记生成用例的场景必须落入至少一条功能用例。
