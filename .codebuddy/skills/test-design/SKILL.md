...
5. 从事实自动生成计划骨架，完整带入动作、具体关联值、稳定结果锚点、分支动作意图、Case级测试点建议、主验证目标建议及已观察状态。每个选择控件的默认选项只需在一个实际有效关联中先切换再切回，其他关联直接使用该值；它们不再额外生成内容重复的“切回”Case，只有状态切换产生独立业务效果时才单独成Case。模型为每个功能一次填写紧凑 `design_context` 和功能级 `automation_profile`，在唯一 `check_assignments` 中分配检查点，并为每个Case确认唯一 `verification_focus`。每个实测有效关联分别形成 baseline Case；未发生关联时仍按每个有限选项和每个有效输入等价类形成独立 baseline Case。空值、格式、边界等已实探DFX分支形成独立 DFX Case；均不默认笛卡尔积。运行时把页面入口自动化层归一为UI，把画像中已有的同类稳定性风险聚合提升为风险项。按钮或 `trigger_element_ref` 本身不是性能依据；只有实测加载、异步、长任务、超时、耗时或需求性能目标时才形成一条轻量响应场景，量化目标必须引用需求或实测事实。模型只补充事实无法确定的专项语义，不按Case展开专项内容。计划按功能 upsert 到同一文件。
6. 严格按计划顺序生成用例，通过内部 `write-cases` 按功能 upsert 到同一 `function-cases.json`。模型只写业务action+expected，并为每条用例填写轻量 `automation_value`、`automation_priority` 和必要的局部覆盖；运行时根据同一关联事实统一注入完整菜单导航、计划标题、Case级 `test_point`、优先级默认值、内部来源、`verification_focus` 和事实引用。动作必须保留该实测关联的具体选项/输入，预期只能使用同一检查的稳定结果锚点，不复制其他关联结论及偶发计数、百分比、耗时或进度；命名测试数据使用 `TEST_*` 引用时，前置条件必须说明同名受控来源。存在实质稳定性风险的自动化结论为“有条件”，不能一律判定适合。重复提交相同功能内容时直接成功且不重复写入。
