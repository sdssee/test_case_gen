...
5. 深探事实交接与原子化：深探准出后，先按测试对象、角色/状态、动作/输入、测试数据、观察点和恢复路径完整交接事实；任一因素实际变化时先形成候选原子场景。每项事实必须形成独立场景、说明合理合并依据，或明确落入性能设计、风险、自动化建议、不适用/不测范围，禁止静默丢失。
...
9. 写入前自查与 Excel 生成：集中检查标题与功能点、编号、入口导航、特殊角色/状态、临时交互闭环、关键步骤与预期、场景/用例 ID 映射及枚举，一次汇总并一次修正，再交给项目统一工具写入；不得用多个 `fix_*` 脚本叠加修补同一结构化数据。正式测试设计只包含 8 个标准 Sheet，不新增 `测试系统导入用例` Sheet，通用 xlsx Skill 只用于读取、渲染和视觉检查。
