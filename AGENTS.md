...
- 功能点作为父场景；形成原子场景时先判定入口，默认使用已认证后的菜单/页面导航，只有登录、重新认证、会话、退出登录或 URL 直达测试使用会话/直达入口。用例从判定后的同一原子场景同步生成 `功能点-当前用例标题`、操作、终态、结果和恢复，再统一编号；普通功能用例把登录状态写入前置条件，不得先生成浏览器或登录步骤再依靠校验删除。环境快照不得成为通用前置或预期。
...
- 唯一草稿只调用 `complete-deliverables`；命令在临时文件中集中校验，通过后立即原子交付。首次失败只允许集中修正唯一数据源一次并再次调用，工具硬限制最多两次校验尝试和一次成功交付；失败即停止，不自动修改、重生成、逐单元格重试或手工降级生成。
