...
- 正式用例只能使用页面可见业务文案和普通测试人员可执行动作；标题、前置、测试数据、步骤、预期中禁止 `uid`、element/node/accessibility ID、DOM/DevTools/MCP/AgentResult、artifacts/orchestration 路径等内部实现标识。
- 截图、截屏、保存证据不是功能测试步骤；证据采集属于 Discovery。预期中禁止“以下任一”“需观察后确认”“待验证”“以实际为准”等不确定 oracle，发现时必须返工 Discovery/Risk。
- 每条用例必须独立完整，不得用“同 TC-xxx”“参照上一条”替代步骤或预期；JSON 字段只能使用冻结中文标准字段，禁止 `title`、`steps`、`expected_result` 等英文别名。
- 测试设计不是执行报告：每条用例的 `执行状态` 必须为 `未执行`，`实际结果` 只能留空或为 `未执行`，不得把 Discovery 观察或其他用例结果填入当前用例。
