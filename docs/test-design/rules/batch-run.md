...
- `batch-scope.json`、`batch-status.csv`、`page-element-inventory.csv`、`page-discovery.csv`、`selection-option-observations.csv`、`interaction-branch-observations.csv`、`configuration-variant-observations.csv`、`element-case-plan.csv`、`test-data-lifecycle.csv` 和 `risk-confirmation.csv` 必须由初始化工具生成或复制标准模板，禁止自定义精简表头或改写产品/批次范围。
