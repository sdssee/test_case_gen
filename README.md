...
一个功能事务可以连续验证多个相关检查点，避免重复扫描页面；但每个有限选项和每个有效输入等价类分别形成独立 baseline Case，空值、格式、边界等已声明分支分别形成独立 DFX Case，不相互合并，也不默认做笛卡尔积。页面模型字段先经过通用归一化：常见输入、选择、触发、切换和容器控件使用统一语义，对象/字符串形式的输入类和选项得到相同结果，未知交互控件不能静默跳过。有效输入类在元素登记时由模型结合页面语义、需求参考和功能推断声明；必填输入同时验证空值，明确结构化格式或边界时验证对应DFX分支。需要按钮产生结果的检查必须包含完整触发动作，并区分主验证控件与辅助使用控件，辅助使用不能替代独立覆盖；两个独立分支也不能复用一次物理操作冒充分别实测。所有页面能力都使用同一套事实与计划规则；CRUD 和配置采用完整业务闭环，配置暂按单因素验证，不做组合爆炸。全部事务结束后只做一次全量扫描，确认没有新增或遗漏控件；后续若又执行事务，旧扫描自动失效。
...
- 页面入口自动化层归一为UI；画像中的同类稳定性风险聚合为一个风险并保留受影响功能，存在实质稳定性风险时自动化适用性写“有条件”。按钮或提交动作本身不构成性能依据；只有实测等待、加载、异步、长任务、超时、耗时或需求性能目标时才形成一条共享的轻量响应场景，量化目标必须引用需求或实测事实，不按Case重复，也不通过后置门禁要求返工。非预期实探结果必须进入用例、风险或性能结论，不能静默丢弃。
