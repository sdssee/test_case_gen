...
- 单个Python建议小于200KB，JSON/CSV/Markdown/TXT建议小于256KB。
- 每个分片写入时完成JSON语法、必填字段、当前分片ID、编号和具体内容的轻量自检。
...
- Review发现问题时只修改相关事实或源分片，再生成双Excel并复查确定性结构；不启动第二轮完整语义Review和自动返工循环。

## 交付

- 使用`complete-deliverables`一次完成样式、导入生成、临时文件验证和可回滚发布。
