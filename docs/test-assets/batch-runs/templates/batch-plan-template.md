...
- 当前批次正式写测试用例前，必须先向用户确认风险项与待确认问题；确认、补充、排除或调整结果必须动态调整测试范围、测试数据、优先级、步骤、预期结果和风险等级。
- 当前批次正式写测试用例前，必须先综合评估 DFX 12 维度 × 4 场景覆盖，明确适用、不适用、待确认和需补充证据的维度，再进入用例设计。
- `batch-status.csv` 和 `page-discovery.csv` 必须复制标准模板或保持与模板完全一致的表头，禁止自定义精简表头、增删列或追加汇总行。
...
- 页面默认全量深探，不把是否深探交给用户确认；创建类操作必须使用带测试标识的数据真实创建成功，编辑/修改页中的每个可修改元素都必须逐项变更，并验证保存后回显与实际生效。
- 风险确认只登记全量深探后模型仍无法理解的业务语义、规则歧义或页面无法观察项。
- 首次交付后的补充、追加、二次补充或页面未覆盖反馈必须作为增量补充建立或更新补充批次，不得只追加用例；补充前识别覆盖缺口、受影响最小标题路径、已有用例 ID 和可复用历史用例，并按最小标题路径重新页面实探目标覆盖缺口。
...
- 正式测试设计、导入文件、批次账本、页面实探记录、临时脚本和 `product-map.xlsx` 都不得保留疑似真实密钥、Token、密码或内部敏感凭据；必须改写为 `<valid_api_key>`、`<test_token>`、`<test_service_url>` 等占位符。
