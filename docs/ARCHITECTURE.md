...
13. 大范围任务建立最小标题批次队列，每个叶子批次使用独立 run-dir 和单行状态账本。每批严格执行 `discovery → plan → risk → cases → delivery`：先全量深探和逐修改项生效闭环，plan 通过后才归纳模型不理解项；只有真实不理解项需要用户确认。`pipeline-status` 从实际资产派生下一步，阶段验证报告以文件、模板、证据和验证器哈希失效下游缓存。所有批次完成后只读各批归档做跨模块汇总，不重新生成各批用例。
