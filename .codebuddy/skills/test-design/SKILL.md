...
5. 深探准出与事实交接：最终复扫新增目标数为 0、动态队列没有待执行/执行中目标、待确认理解问题为空后，运行 `python scripts/test_design_excel_tools.py validate-discovery --discovery-state <discovery-state.json>`；失败时一次处理汇总问题并继续深探，不自动重试。通过后按测试对象、角色/状态、动作/输入、测试数据、观察点和恢复路径形成候选原子场景，为每项事实填写去向与关联 ID，禁止静默丢失。
...
8. 用例设计：按小功能块连续编排并直接复用原子场景；先确定功能点并同步生成 `功能点-当前用例标题`，再按角色/状态生成系统入口，按 `入口 → 导航 → 前置状态/造数 → 目标操作 → 状态变化 → 终态动作 → 页面/数据结果 → 恢复/清理` 一次形成闭环。未登录、无权限、异常网络等场景按自身状态进入，不得机械补登录；下拉浮层以选择选项、浮层收起和结果更新闭环。步骤和预期内容完成后分别一次连续编号，不在后续补丁中硬编码编号。每个标记生成用例的原子场景必须落入至少一条功能用例。
...
- 用例字段必须按同一原子场景同步构造，先完成内容再统一编号；禁止给未登录、无权限或异常环境用例机械追加普通登录步骤。
