...
3. 每个批次正式写测试用例前，必须把当前批次最小标题路径和页面或功能点实探结果写入 `page-discovery.csv`。
   `batch-status.csv`、`page-discovery.csv`、`element-case-plan.csv` 和 `test-data-lifecycle.csv` 必须复制 `templates/` 中的标准模板或保持与模板完全一致的表头，禁止自定义精简表头、增删列或在末尾追加汇总行。
...
4. 页面深探后必须生成 `element-case-plan.csv`，功能测试用例必须从元素计划派生；真实新增、编辑、删除或用户提供测试数据必须同步 `test-data-lifecycle.csv`，配置项保存类用例必须记录保存后回显和生效验证。
