# 测试资产归档与模块能力索引规范（历史参考）

> 本文件保留旧版资产结构说明，不再作为执行入口。当前执行以`docs/test-design/rules/README.md`及其路由的专题规则为准；冲突时以当前专题规则为准。
...
页面实探运行保留`batch-status.csv`、`page-discovery.csv`和`artifacts/`，首次写入事实时由工具自动创建目录。多页面、多模块或超过一个最小标题路径时额外保留精简`batch-plan.md`。不预创建空白Review；全部交付完成后只创建一次`final-review.md`。`batch-status.csv`只承担进度、数量、路径和断点恢复，`page-discovery.csv`承载带事实状态的页面记录。
...
对页面既有数据必须主动做只读深探：可以打开详情、进入编辑/修改页面观察字段和联动、尝试点击删除/停用/提交等危险按钮以打开确认弹窗或保存前校验，但不得点击最终确认、保存、提交或改变既有数据状态；必须记录确认弹窗、二次确认文案、权限提示、保存前校验、取消路径和返回路径。可以复制既有数据字段作为参考，改名或改编码为带 `AI_TEST`、`CODEX_TEST`、当前日期或任务编号的新数据后执行新增，用新创建的数据继续探索详情页、下一级页面、编辑、删除、状态变更和失败分支。
...
测试用例必须详细覆盖每个功能自身的baseline Case和适用DFX场景；不适用方向不机械展开，不默认生成配置或选项组合矩阵，也不能用一个笼统用例替代多个独立可验证功能。
