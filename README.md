...
- 页面入口自动化层归一为UI；画像中的稳定性风险直接去重进入风险项；存在提交、加载或超时事实时只形成一条共享的轻量响应场景，不按Case重复，也不通过后置门禁要求返工。
- Excel组装从最终用例和主验证目标派生场景观察点，使用计划中的真实风险和专项结论，元素覆盖只显示精简分支摘要，按实际列宽计算行高，并校验跨Sheet一致性及无阻塞未覆盖元素。
