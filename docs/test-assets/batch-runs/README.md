...
4. 必须先默认全量深探并回写 `page-discovery.csv`；创建类操作必须真实创建成功，编辑/修改类中的所有可修改元素必须作用于本次创建的测试数据并逐项验证保存回显和实际生效。之后仅把模型不理解项写入 `risk-confirmation.csv`，确认完成后才能生成测试用例。真实新增、编辑、删除或用户提供测试数据必须同步 `test-data-lifecycle.csv`。
