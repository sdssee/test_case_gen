...
5. 深探准出与事实交接：最终复扫新增目标数为 0、动态队列没有待执行/执行中目标、待确认理解问题为空后，运行 `python scripts/test_design_excel_tools.py validate-discovery --discovery-state <discovery-state.json>`；失败时一次处理汇总问题并继续深探，不自动重试。通过后进入测试设计阶段，重新读取并执行 `case-design.md` 的“实探证据到通用设计”，先将事实抽象为可重复的角色、状态、动作、数据条件、观察点和恢复路径，再形成候选原子场景并填写去向与关联 ID，禁止静默丢失或直接复制环境快照。
...
8. 用例设计：按小功能块连续编排并复用已经完成证据抽象的原子场景；先确定功能点并同步生成 `功能点-当前用例标题`，再按角色/状态生成系统入口，按 `入口 → 导航 → 前置状态/造数 → 目标操作 → 状态变化 → 终态动作 → 页面/数据结果 → 恢复/清理` 一次形成闭环。每条用例独立构造和清理测试数据，不依赖实探遗留对象、具体运行账号或其他用例结果；未登录、无权限、异常网络等场景按自身状态进入，不得机械补登录；下拉浮层以选择选项、浮层收起和结果更新闭环。步骤和预期内容完成后分别一次连续编号，不在后续补丁中硬编码编号。每个标记生成用例的原子场景必须落入至少一条功能用例。
9. 写入前自查与 Excel 生成：集中检查标题与功能点、编号、入口导航、特殊角色/状态、临时交互闭环、关键步骤与预期、场景/用例 ID 映射及枚举，一次汇总并一次修正；页面任务在同一状态文件集中补全 `scenario_case_mapping`，调用 `complete-deliverables` 时传入 `--discovery-state`。失败时停止并一次处理汇总问题，不自动循环生成，不得用多个 `fix_*` 脚本叠加修补同一结构化数据。正式测试设计只包含 8 个标准 Sheet，不新增 `测试系统导入用例` Sheet，通用 xlsx Skill 只用于读取、渲染和视觉检查。
