...
17. 每次正式交付都由 `scripts/test_design_excel_tools.py complete-deliverables` 通过统一表头映射链路同步生成测试系统导入文件；`generate-import` 只用于维护或重新转换已有正式测试设计。临时脚本只能调用统一工具或复用同等映射函数，禁止按固定列序号数组写入模板；生成后必须通过 `-ImportWorkbookPath` 校验字段错位、下拉框、自动字段空值、模板数据验证、多行换行样式和 DFX 标签落地。
