...
8. 用例设计：按小功能块连续编排，直接复用原子场景和深探闭环；每个标记生成用例的原子场景必须落入至少一条功能用例，编号连续，关键操作与预期按执行顺序对应，标题、数据、步骤和预期保持同一事实链。
9. 写入前自查与 Excel 生成：对结构化内容集中执行一次中文自查并一次修正，再交给项目统一工具写入；正式测试设计只包含 8 个标准 Sheet，不新增 `测试系统导入用例` Sheet，通用 xlsx Skill 只用于读取、渲染和视觉检查。
