...
深遍历下拉框、级联选择、单选框、复选框、树选择、枚举筛选等选择类控件时，不得只展开查看选项；有限集合必须覆盖每个选项，可选项实际选择，真实禁用项尝试并记录具体阻塞状态与独立证据，全部逐行写入 `selection-option-observations.csv`；动态集合必须记录搜索、滚动/分页、边界和清空策略，观察后续字段、二级/三级下拉、列表数据、按钮禁用态、校验提示、默认值、清空重置、搜索结果和提交前提示的变化。对存在前后依赖的控件，必须在 `page-discovery.csv` 汇总 `选项取值/输入值` 和 `联动/依赖变化`；无联动也要明确记录为无联动变化。

能通过页面点击、选择、输入、切换、翻页或观察状态验证的问题必须由模型自行操作验证；页面验证未完成时必须退回 discovery，只有依赖接口、日志、数据库、异步任务、额外权限或第三方系统且模型仍不理解的内容才提交用户确认。

不同用例不得复用相同的 `操作步骤 + 预期结果`；标题中的选项值、页码、每页条数和状态值必须在步骤与预期中落地。JSON 分片、正式 8 Sheet 工作簿与独立导入工作簿之间必须逐字段一致。
