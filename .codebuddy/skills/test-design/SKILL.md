...
4. 一个功能事务完成后原子记录多个 `checks`；从 `events.jsonl` 编译七类紧凑事实到 `facts.json`。证据仅作可选诊断，不参与完成判断和后续生成。
5. 从事实识别独立功能，先建立基线意图，再按实际约束左移展开 DFX，通过内部 `write-plan` 原子写入 `case-plan.json`。一个事务可形成一个或多个 Case，不按点击数机械拆分。
6. 严格按计划顺序生成用例，通过内部 `write-cases` 原子写入 `function-cases.json`。步骤使用 `action+expected` 配对；每条用例第一步采用完整菜单路径，例如“进入告警管理-告警列表”。生成时发现结构问题只修当前内容，不留到 Review 才拒绝。
