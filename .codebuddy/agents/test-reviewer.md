...
- 审计实探完整性、交互闭环、证据真实性、CRUD 生效与数据安全、实探→计划→用例一致性、功能点集中、步骤/预期唯一性、逐元素 DFX 12×4、分片/manifest 和交付前门禁。
- 必须从冻结的实际 JSON、CSV、traceability、manifest 和证据重新计算数量与一致性，禁止相信其他 Agent 的 `gate_summary`、口头总数或历史结论；空标题、UID/内部路径、截图步骤、不确定预期、非 `未执行` 状态、伪造实际结果、分片数量失真、manifest 漏 ID 任一存在即阻断。
