...
- 粗遍历只确定模块位置、入口、依赖和深探范围。页面任务只维护一份 `discovery-state.json`；基线扫描、动态深探、最终复扫和 `validate-discovery` 全部通过后才能进入 DFX。
...
- 以功能点为父场景，用例从同一原子场景同步形成标题、入口导航、步骤、终态、结果和恢复；环境快照转换成业务角色、当前用例造数和相对断言。
...
- 写入前只做一次集中自查并修正唯一结构化数据源。`complete-deliverables` 一次生成、一次联合质检；失败停止，不自动修改、循环重试、重生成或手工降级。
