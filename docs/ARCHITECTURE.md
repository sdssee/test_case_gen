...
1. 正式测试设计 Excel 只保留 7 个标准 Sheet，不新增 `测试系统导入用例` Sheet。
2. Skill、Rule、AGENTS、CODEBUDDY 是轻入口，目标低于 10000 字符；任务入口只加载路由和硬规则，详细规则拆到 `docs/test-design/rules/` 并在深探、场景/用例、DFX、Excel 交付等对应阶段按需读取，同一阶段连续执行不重复加载。
...
11. 当用户要求为某个模块生成测试设计时，正式写用例前必须先做模块级粗遍历，识别菜单入口、页面清单、核心功能点、业务对象、状态流转和跨模块依赖；具体写测试用例时，再在对应页面或功能点内做深遍历，有可访问页面时使用浏览器或 computer use 覆盖所有可点击、可输入、可测试元素以及所有可点击/可交互功能点。选择值、输入值、联动变化和结果分支使用模板已有的 `预期行为`、`业务依据/规则来源` 与 `待确认问题/备注` 记录，不新增不存在的覆盖清单字段。新增类流程必须实填实走，成功时进入详情页、下一级页面或后续配置页继续观察，失败时记录真实失败提示、停留页面和可恢复路径。
12. 每一批测试设计都必须严格执行完整 test-design Skill 和 Rule，不得因为分批而降级；每批都必须覆盖功能测试、性能测试、异常流程、边界值、权限/角色、状态流转、数据一致性、兼容性/稳定性、风险与待确认问题和页面元素覆盖清单。
...
17. 每次正式交付都由 `scripts/test_design_excel_tools.py complete-deliverables` 从正式模板副本重建测试设计并同步生成测试系统导入文件；页面任务先以单份临时 `discovery-state.json` 通过深探准出，并在交付时复用该文件核对事实、场景和用例映射。`generate-import` 只用于维护或重新转换已有正式测试设计。临时脚本不得直接写正式 Excel；统一工具只在临时文件中生成和校验，全部通过后才写入交付目录，失败时保留原交付件。
