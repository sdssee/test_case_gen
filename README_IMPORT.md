...
首次交付后如果用户要求补充、追加、二次补充某类测试用例或指出页面未覆盖，必须按增量补充处理，不得只追加用例；补充前读取现有交付件、页面元素覆盖清单、page-discovery.csv、batch-status.csv、product-map.xlsx 和归档测试设计，识别覆盖缺口、受影响最小标题路径、已有用例 ID 与可复用历史用例，再建立补充批次并重新页面实探目标覆盖缺口。二次补充完成后同步正式测试设计、独立导入文件副本、内部归档和产品版图。
...
下拉框、级联选择、单选框、复选框、树选择、枚举筛选等选择类控件不得只展开查看选项，必须分别选择代表性选项，观察并记录后续字段、二级/三级下拉、列表数据、按钮禁用态、校验提示、默认值、清空重置、搜索结果和提交前提示的变化；相关结果写入 page-discovery.csv 的 选项取值/输入值 和 联动/依赖变化。
输入框、搜索框、文本域、数字框、日期框、URL/地址、端口、邮箱、手机号、名称、编码等输入类控件不得只观察字段存在，必须实际输入正常、异常、边界或用户提供的测试数据，触发查询、下一步、测试连接、预览、保存前校验等后续动作，并记录真实提示、结果分支/后续状态和可恢复路径；相关结果写入 page-discovery.csv 的 选项取值/输入值、预期/观察行为 和 结果分支/后续状态。
新增、创建、添加、新建、保存、提交、下一步、完成、测试连接等新增类流程必须实填实走，在安全边界内实际填写测试数据并触发后续动作；成功时进入详情页、下一级页面或后续配置页继续观察，失败时记录真实失败提示、停留页面和可恢复路径；相关结果写入 page-discovery.csv 的 选项取值/输入值、预期/观察行为 和 结果分支/后续状态。
...
交付件校验会拦截 batch-status.csv/page-discovery.csv 自定义精简表头、CSV 字段错位、batch-plan.md 状态或页面数不一致、product-map.xlsx 仍保留示例产品/示例模块/示例页面、用例资产索引和页面元素地图未覆盖正式 Excel，以及疑似真实密钥、Token、密码未替换为 <valid_api_key>、<test_token>、<test_service_url> 等占位符的问题。

测试系统导入文件请优先使用统一工具生成，避免字段错位和样式丢失：
...
导入文件生成后，请追加 `-ImportWorkbookPath <导入文件.xlsx>` 运行交付件校验，确认测试系统模板下拉框、自动字段空值、字段映射和多行换行样式都保留正确。
...
对既有数据必须主动做只读深探：可以进入详情、编辑页、删除/停用/提交确认弹窗观察字段、联动、二次确认、权限提示和取消路径，但不能最终确认；可以复制既有数据的非敏感字段，改名或改编码为带测试标识的新数据后新增，再用新数据探索后续页面。
