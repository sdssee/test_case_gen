...
15. 初步风险必须先分为页面/DOM可验证、测试数据可验证、外部条件依赖和业务决策；先完成可验证项的定向补探，只有仍无法验证或需要业务取舍的事项才提交用户确认。
16. 用户确认、补充、排除或调整后必须判断是否产生新的验证目标；可实探时先返回定向补探并更新风险与覆盖状态，再动态调整测试范围、测试数据、优先级、步骤、预期结果和风险等级。
