...
项目级测试资产事实索引。用于记录已归档测试设计、模块能力、跨模块依赖、可复用测试数据和变更记录。AI 每次生成前应读取该索引，不能只依赖对话记忆。
...
每次生成前请读取 docs/test-design/indexes/module-capability-index.xlsx；如果当前模块依赖其他模块，请读取索引中登记的依赖模块归档测试设计。最终版测试设计应回存 docs/test-design/outputs/，导入文件副本回存 docs/test-design/imports/，并同步更新模块能力索引、跨模块依赖和变更记录。
