...
```json
{
  ...
  "checks": {
    "design_context": "pass",
    "scenario_diversity": "pass",
    "action_expected": "pass",
    "dfx_fit": "pass",
    "performance_risk": "pass",
    "automation": "pass",
    "source_claims": "pass",
    "internal_prose": "pass"
  },
...
运行时把确定性检查与该语义判断合并写入 `review.json`。只有两部分同时通过才可交付；发现页面事实确实缺失时明确标记阻塞，其他问题只指出单个功能或Case的局部修正，不自动循环。
