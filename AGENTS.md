...
- 粗遍历只定位模块、入口、依赖和深探范围。页面任务维护单份 `discovery-state.json` 动态队列；深探执行基线盘点、业务流程补充和最终复扫，进入 DFX 前必须通过 `validate-discovery`。
...
- 每次正式交付使用 `complete-deliverables`；工作簿可解析后一次汇总正式工作簿、深探映射和导入文件问题。失败即停止，不自动修改、重生成、逐单元格重试或手工降级生成。
