...
10. **一次交付**：唯一草稿先调用 `preflight-deliverables` 集中预检，首次失败只允许集中修正唯一数据源一次；通过后调用一次 `complete-deliverables`，页面任务始终传入同一 `--discovery-state`。工具硬限制最多两次预检和一次正式交付；失败停止，不自动修改、重生成或叠加 `fix_*`。
