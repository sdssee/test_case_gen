...
2. 粗遍历和摘要：模块或大范围任务先做菜单轮廓、页面清单、核心功能点、业务对象、状态流转、跨模块依赖识别，并形成产品理解摘要或模块理解摘要和初步风险；粗遍历不能替代深探。
3. 风险分流、页面深探与定向补探：将风险分为页面/DOM可验证、测试数据可验证、外部条件依赖和业务决策；有页面、原型或窗口时深遍历当前范围全部可交互元素，并完成可验证风险的定向补探，只把仍无法验证或需要业务取舍的事项发给用户确认。
4. 用户补充回环：根据用户确认、补充、排除或调整动态调整测试范围、测试数据、优先级、步骤、预期结果和风险等级；用户提出需验证或新增验证目标时，先返回定向补探并更新风险与覆盖状态。
5. DFX 覆盖评估：确认不存在页面可访问且可操作的待实探事项后，综合产品理解、页面实探、文档、历史资产、测试数据和风险项，评估 DFX 12 维度 × 4 场景的适用、不适用、待确认和需补充证据结论。
...
- 深探完成必须以全部可交互元素已有处理结果为准，不能主观以“信息足够”提前结束；单个操作失败最多重试一次并尝试 DOM、快照、可见文案或无障碍信息，仍失败才记录原因。
