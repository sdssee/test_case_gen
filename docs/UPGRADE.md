...
外网维护框架规范、模板、脚本和校验规则；内网保留历史归档、导入副本和客户交付件。
...
迁移脚本必须随升级包一起提供。内网升级脚本会在复制框架文件前检查升级包中是否存在对应迁移脚本；如果未传入 `-RunMigrations`，或升级包缺少迁移脚本，脚本会停止且不复制文件。迁移脚本必须读取并保留旧资产，只做增量变更。
