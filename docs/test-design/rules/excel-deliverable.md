...
完整交付只使用 `complete-deliverables`。命令从唯一草稿在临时文件中完成模板重建、导入文件生成和集中校验，通过后立即原子写入正式路径；页面任务传入同一 `--discovery-state`。首次失败后只允许回到唯一结构化数据源集中修正一次并再次调用，第二次仍失败必须停止。工具从批次状态硬限制最多两次校验尝试和一次成功交付，禁止自动重试、手工降级生成或叠加 `fix_*`。

传入 `--formal-workbook` 的模板填充草稿必须位于本批临时目录，不得预先写入 `deliverables/`；统一工具校验通过后再原子写入正式交付目录，任务结束按明确路径清理草稿。

集中校验在工作簿结构可解析后必须继续执行正式工作簿、深探映射和导入文件的全部独立检查，一次返回完整阻塞问题，不截断问题项；只有文件不存在、文件损坏、Sheet 或核心表头缺失等使后续检查无法可靠执行的结构错误可以立即停止。校验不自动修改、重生成或循环调用自身，通过后不重复质检。
