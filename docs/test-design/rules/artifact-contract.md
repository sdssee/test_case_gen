...
计划只写测试意图和唯一检查点分配账本；`fact_refs`、`scenario_signature`、Case级 `test_point`、元素覆盖、功能覆盖和DFX关联由系统派生，不要求模型重复维护。无歧义的一对一机械分配可由运行时补齐。每个实测有效关联、有限选项和有效输入等价类分别对应baseline Case，空值、无效格式、边界等对应DFX Case，不做笛卡尔积。性能按功能 `interaction_profile` 聚合：网络读取、写入、异步、批量传输、数据渲染形成三档实际基线，只有本地静态交互不适用；量化目标仍必须有需求或实测依据。
...
discovery、plan、cases在各自写入现场已完成确定性约束。Review只检查当前产物指纹和跨产物映射，再与一次语义判断合并写入 `review.json`；不重复前序门禁、不新增工作、不自动循环。
