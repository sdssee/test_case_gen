...
17. 每次正式交付只调用 `complete-deliverables`：命令从正式模板副本重建测试设计、生成导入文件并集中校验，通过后立即写入交付目录；页面任务先以单份临时 `discovery-state.json` 通过深探准出并复用该文件核对事实、场景和用例映射。首次失败只允许集中修正一次，工具硬限制最多两次校验尝试和一次成功交付；`generate-import` 只用于维护，失败时保留原交付件。
