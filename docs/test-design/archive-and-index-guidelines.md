...
当范围超过一个二级菜单时，禁止直接生成完整测试用例；必须先建立批次队列，按批次逐个执行完整测试设计。每批完成覆盖质量自检并回存内部资产后，才能进入下一批。覆盖质量自检至少确认页面元素覆盖清单、功能测试用例、性能测试设计、异常/边界/权限/状态/数据一致性用例、风险与待确认问题、自动化建议和产品版图回存均已完成。所有批次完成后只做跨模块汇总、回归范围、风险清单和客户总览，不得重新一次性改写各批完整用例。
